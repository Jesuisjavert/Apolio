...
public class CommentController {
    ...
    @ApiOperation(value = "유저 번호, 게시물 번호, 댓글 내용을 받아서 댓글에 입력시킨다", response = String.class)
    @PostMapping(value = "/comment")
    public ResponseEntity<String> insertComment(@RequestBody CommentForm commentForm){
        Long check = commentService.comment(Long.parseLong(commentForm.getAccount_id()), Long.parseLong(commentForm.getArticle_id()), commentForm.getContent());
        if(check != 0){
    ...
    @ApiOperation(value = "게시물 번호에 해당하는 댓글들을 조회한다", response = List.class)
    @GetMapping(value = "/comment/{article_id}")
    public ResponseEntity<List<Comment>> listComment(@PathVariable Long article_id){
        List<Comment> commentList = commentService.findCommentById(article_id);
